...
4. Запрос на единственный бэкенд: `POST https://imageprompt.tools/api/extension/analyze`, тело `{ image_base64, style? }`. Бэкенд — новый Next.js route на лендинге (без auth), внутри: rate-limit по IP (30 запросов/сутки), вызов **Gemini 2.5 Flash vision** с системным промптом, производным от `pingan8787/image2prompt/src/prompts/generalPrompt`. Ответ `{ prompt: string }`.
...
| Style options | `Photo-real`, `Midjourney`, `Stable Diffusion`, `Flux` |
