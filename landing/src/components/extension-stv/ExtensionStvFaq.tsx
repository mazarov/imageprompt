...
export async function ExtensionStvFaq() {
  ...
  return (
    <section className={LANDING_SECTION_Y} aria-labelledby="extension-stv-faq-heading">
      <div className="mx-auto max-w-3xl px-4 sm:px-6">
        <h2
          id="extension-stv-faq-heading"
          className="text-center text-2xl font-bold tracking-tight text-zinc-50 sm:text-3xl"
        >
          <span aria-hidden className="mr-2 inline-block select-none">
            ❓
          </span>
          {t("title")}
        </h2>
        ...
        <div className="mt-8 space-y-3">
          {FAQ_KEYS.map((item) => (
            <details
              key={item.q}
              className={`group rounded-xl ${LANDING_BORDER_CARD} ${LANDING_SURFACE_CARD} transition-colors open:border-indigo-500/25 open:bg-[rgb(24_24_27/0.5)] open:ring-1 open:ring-indigo-500/15`}
            >
              ...
              <div className="px-5 pb-4 pt-0 text-sm leading-relaxed text-zinc-400 [&_a]:text-indigo-300 [&_a]:underline-offset-2 [&_a]:hover:underline">
                <div className="pt-2">{t(item.a)}</div>
              </div>
            </details>
          ))}
        </div>
